...
const gestureMixin = {
  ...
  methods: {
    ...
    setGestureState(params) {
      ...
      this.gesture = { ...this.gesture, ...params }
    },
    getGestureState() {
      if (!this.gesture) {
        return this.gesture
    ...
    checkIfMultiTouchMove() {
      ...
      if (rotate) {
        const rotation = calcRotation(startMutliFingerStatus,
          mutliFingerStatus)
        this.setGestureState({
          rotation
        })
    ...
    updateGestureStatus(e) {
      ...
      const moveStatus = calcMoveStatus(startTouches, touches, time -
        startTime)
